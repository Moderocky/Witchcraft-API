...
public class PlanarBanishmentSpell extends StandardSpell {
    ...
    public void run(LivingEntity caster, int range, float scale, double amplitude) {
        ...
        if (data.isMainWorld() || data.safe) {
            caster.sendMessage(Component.text("You cannot banish somebody from the Mortal Realm.", WitchcraftAPI.colors()
                .lowlight()));
            return;
        }
    ...
    protected void doBanish(Player player, World world) {
        ...
        player.sendMessage(Component.text("You were banished from this realm...", WitchcraftAPI.colors().lowlight()));
