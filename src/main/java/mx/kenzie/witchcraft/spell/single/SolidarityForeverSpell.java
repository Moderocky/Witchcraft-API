...
public class SolidarityForeverSpell extends AbstractTeleportSpell {
    ...
    public void run(LivingEntity caster, int range, float scale, double amplitude) {
        ...
        final Location centre = caster.getEyeLocation().add(0, 2.5, 0).setDirection(new Vector(0, -1, 0));
        final MalleablePortal portal = WitchcraftAPI.minecraft.netherPortal(centre);
        assert portal != null;
        final Entity entity = portal.getBukkitEntity();
        Bukkit.getScheduler().runTaskLater(WitchcraftAPI.plugin, entity::remove, 20 * 8L);
        if (coven == null) {
